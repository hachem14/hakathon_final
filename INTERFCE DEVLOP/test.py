...
class AccueilMedsafe(ctk.CTk):
    ...
    def _creer_carte_profil(self, icone, titre, sous_titre, couleur, hover, cmd):
        ...
        # Contenu à l'intérieur du bouton (via frame superposée)
    ...
    def ouvrir_medecin(self):
        ...
        try:
            ...
            login = LoginWindow()
            login.mainloop()
            if login.connexion_reussie:
                dash = DashboardMedecin(login.medecin_data)
                dash.mainloop()
        except ImportError as e:
            import tkinter.messagebox as mb
            mb.showerror("Fichier manquant",
                         f"Le fichier dashboard_medecin.py est introuvable.\n\n{e}")
        finally:
            self.deiconify()
    ...
    def ouvrir_pharmacien(self):
        self.withdraw()
        try:
            from dashboard_pharmacien import LoginPharmacien, DashboardPharmacien
            login = LoginPharmacien()
            login.mainloop()
            if login.connexion_reussie:
                dash = DashboardPharmacien(login.pharmacien_data)
                dash.mainloop()
        except ImportError as e:
            import tkinter.messagebox as mb
            mb.showerror("Fichier manquant",
                         f"Le fichier dashboard_pharmacien__1_.py est introuvable.\n\n{e}")
        finally:
            self.deiconify()
    ...
    def ouvrir_fournisseur(self):
        self.withdraw()
        try:
            from dashboard_fournisseur import LoginFournisseur, DashboardFournisseur
            login = LoginFournisseur()
            login.mainloop()
            if login.connexion_reussie:
                dash = DashboardFournisseur(login.fournisseur_data)
                dash.mainloop()
        except ImportError as e:
            import tkinter.messagebox as mb
            mb.showerror("Fichier manquant",
                         f"Le fichier dashboard_fournisseur.py est introuvable.\n\n{e}")
        finally:
            self.deiconify()
